...
uint8_t time_duration=0; // in minuties
uint16_t run_time = 0; 
...
void SegDis_show(){
    ...
    if(power_ok)
    {
        ...
        if(run_flag == 1)
        {
            if(run_mode == 0)
            {
                screen_num[0]=hexDis[wash_temperature/10];
                screen_num[1]=hexDis[wash_temperature%10];
                if(time_light == 0)
                {
            ...
            else if(run_mode == 1)
            {
                ...
                screen_num[1]=hexDis[temperature/10%10];
                screen_num[2]=hexDis[temperature%10];
                screen_num[3]=hexDis[10];
...
void temp_check()
{
    ...
    temperature = 15+ (uint8_t)(temp.read()*100.00);
